...
//RANGE
// const priceSlider = document.querySelector('.price-filtr__slider');
// if (priceSlider) {
...
$(document).on('keydown', function (event) {
	if (event.keyCode == 27) {
		$('.panel__form').removeClass('_active');
	}
});
// Dynamic Adapt v.1
...
// Вывод изображений слайдера в булет
...
// subSlider в связке с productSlider
...
// productSlider
if (document.querySelector('.images-product__productSlider')) {
	let imagesProductSlider = new Swiper('.images-product__productSlider', {
		observer: true,
		observeParents: true,
		slidesPerView: 1,
		spaceBetween: 0,
		speed: 800,
		thumbs: { // связать слайдеры
...
// Выпадает саб-меню с продуктом при клике только на мобильном девайсе
if (isMobile.any()) {
	...
	// При наведении выпадает саб-меню с продуктом только на ПК
} else {
	...
	for (let index = 0; index < menuParents.length; index++) {
		...
		menuParent.addEventListener("mouseenter", function (e) {
			...
			$('.submenu-page').toggleClass('_active');
		});
		menuParent.addEventListener("mouseleave", function (e) {
			menuParent.classList.remove('_active');
			$('.submenu-page').removeClass('_active');
		});
	}
}
...
// Выбор категорий и сумирование их с выводом кол-ва
